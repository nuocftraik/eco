    ...
    where T : class, IAggregateRoot
{
    ...
    public async Task DeleteRangeAsync(ISpecification<T> specification, CancellationToken cancellationToken)
    {
        // Lấy danh sách các thực thể cần xóa theo specification
        var entities = await _decorated.ListAsync(specification, cancellationToken);

        // Thêm sự kiện EntityDeletedEvent cho từng thực thể
        foreach (var entity in entities)
        {
            entity.DomainEvents.Add(EntityDeletedEvent.WithEntity(entity));
        }

        // Xóa các thực thể thông qua repository trang trí
        _decorated.DeleteRangeAsync(entities, cancellationToken);
    }
